package br.edu.fatec.aula01.fuelcalculatorapp;
...
public class MainActivity extends AppCompatActivity {
    ...
    private EditText ethanolPriceVEditText;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ethanolPriceVEditText = findViewById(R.id.ethanol_price_edit_text);
        gasPriceEditText = findViewById(R.id.gas_price_edit_text);
        ethanolMilageEditText = findViewById(R.id.ethanol__milage_edit_text);
        ...
        calculatorButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (validateFields()){
                    changeToResulActiviry();
                }
            }
        });
    }

    private void changeToResulActiviry(){
        Intent intent = new Intent(this, ResultActivity.class);

        String ethanolPriceStr = ethanolPriceVEditText.getText().toString();
        ...
        intent.putExtra("ETHANOL_MILAGE", gasMilage);

        startActivity(intent);
    }
    private boolean validateFields(){
        boolean correctValidation = true;
        if (ethanolPriceVEditText.getText().toString().length() == 0){
            //ethanolPriceVEditText.setError("Digite o valor do etanol");
            ethanolPriceVEditText.setError(getString(R.string.erro_price_etanol));
            correctValidation = false;
        }
        if (gasPriceEditText.getText().toString().length() == 0){
            //gasPriceEditText.setError("Digite o valor do gasolina");
            gasPriceEditText.setError(getString(R.string.erro_price_gas));
            correctValidation = false;
        }
        if (ethanolMilageEditText.getText().toString().length() == 0){
            //ethanolMilageEditText.setError("Digite o valor do etanol milage");
            ethanolMilageEditText.setError(getString(R.string.erro_milage_etanol));
            correctValidation = false;
        }
        if (gasMilageEditText.getText().toString().length() == 0){
            //gasMilageEditText.setError("Digite o valor do gasolina milage");
